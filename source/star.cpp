...
#define INIT_BLOCK_SCALE		D3DXVECTOR3(1.0f, 1.0f, 1.0f)

#define	BLOCK_WIDTH				(16.0f)						// 弾の半径幅
#define	BLOCK_HEIGHT			(4.0f)						// 弾の半径幅
#define BLOCK_DEPTH				(1.0f)
...
void UpdateStar(void)
{
	...
	if (!(cntFPS++ % interval))
	{
		//SetStar(D3DXVECTOR3(cosf(anglePos) * length, 90.0f + fabsf(sinf(anglePos) * length/3.0f), 500.0f), 
	...
	{// 画面外に出たの星を初期化する
		...
		for (int cntBlock = 0; cntBlock < MAX_BLOCK; cntBlock++)
		{
			...
			if (wkStar->block[cntBlock].pos.z < GetGameCameraPos().z)
			{
				ResetStar(wkStar);
			}
...
HRESULT InitBlock(LPDIRECT3DDEVICE9 device, STAR* wkStar)
{
	...
	for (int i = 0; i < INIT_NUM_BLOCK; i++)
	{
		...
		wkStar->block[i].rot = D3DXVECTOR3(0.0f, 0.0f, D3DX_PI / 5.0f * i);
...
void SetLongNote(float speed)
{
	SetStar(SetPos, D3DXVECTOR3(0.0f, 0.0f, speed));
...
bool ClippingStar(PLANE section)
{
	...
	for (int i = 0; i < MAX_STAR; i++)
	{
		if (wkLen < star[i].block[0].pos.z && 0.0f < star[i].block[0].pos.z)
		{
			wkLen = star[i].block[0].pos.z;
			cntStar = i;
		}
	}

	//if (star[cntStar].block[0].pos.z < 2.2f &&star[cntStar].numBlock == 1 && checkCut)
	//{
	//	if (ClippingStar(wkCutter))
	//	{
	//		checkCut = false;
	//	}
	//	//continue;
	...
	for (int i = 0; i < MAX_STAR; i++, cntStar++)
	{
		cntStar = cntStar % MAX_STAR;
